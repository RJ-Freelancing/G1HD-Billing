      ...
      <Wrapper>
        <ProfileEditWrapper elevation={24}>
          ...
          <form onSubmit={this.updateProfile} style={{padding: 10}}>
              ...
              >
                {(inputProps) => (
                  <TextField 
                    ...
                    fullWidth
                    disabled={this.props.loading}
                    error={Boolean(this.state.phoneNo) && !this.state.phoneNo.match(validPhoneNo)}
                    helperText={this.state.phoneNo && !this.state.phoneNo.match(validPhoneNo) ? "Invalid Phone" : null}
                  />
                )}
              </InputMask>
              <TextField
            ...
            <Button 
              ...
              type="submit" 
              color="primary" 
              disabled={this.props.loading || this.checkValidation() || isEqual(this.props.auth, this.state)}
            >
              Update
        ...
        <ProfileDetailsWrapper elevation={24}>
          ...
          <ProfileDetails>
            ...
            <Typography variant="body2">{format(Date.parse(this.state.createdAt), 'd MMM YYYY @ HH:mm:ss')}</Typography>
            <Typography variant="subtitle2">Last Updated</Typography>
            <Typography variant="body2">{format(Date.parse(this.state.updatedAt), 'd MMM YYYY @ HH:mm:ss')}</Typography>
          </ProfileDetails>
